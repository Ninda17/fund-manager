  ...
  const fetchRequests = async () => {
    try {
      ...
      const url = statusFilter
        ? `${API_PATHS.ADMIN.GET_REALLOCATION}?status=${statusFilter}`
        : API_PATHS.ADMIN.GET_REALLOCATION;
      ...
      setError(
        error.response?.data?.message || "Failed to load reallocation requests."
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((req) => {
        ...
        const userName = (req.requestedBy?.name || "").toLowerCase();
        const userEmail = (req.requestedBy?.email || "").toLowerCase();
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <p className="text-gray-600 text-sm sm:text-base">
            View all fund reallocation requests
          </p>
        </div>
        ...
        {allRequests.length > 0 && (
          <div className="mb-6">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search by project IDs, amounts, reasons, status, type, or user..."
                value={searchQuery}
              ...
              <p className="mt-2 text-sm text-gray-600">
                {requests.length === 0 ? (
                  ...
                  <span>
                    ...
                    {requests.length === 1 ? "request" : "requests"} matching "
                    {searchQuery}"
                  </span>
                )}
              </p>
                    ...
                    {requests.map((request) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm">
                            <div className="font-medium text-gray-900">
                              {getSourceDisplay(request)}
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className="text-sm text-gray-900">
                            {getDestinationDisplay(request)}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm">
                            ...
                            {request.sourceCurrency !==
                              ...
                              <div className="text-gray-500 text-xs">
                                →{" "}
                                {formatCurrency(
                                  request.convertedAmount || request.amount,
                                  request.destinationCurrency
                                )}
                              </div>
                            )}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`inline-flex items-center px-4 py-1.5 rounded-full text-xs font-semibold border ${getStatusBadgeStyle(
                              request.status
                            )}`}
                          >
                            {request.status}
...
export default AllReallocation;
