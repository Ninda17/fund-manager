...
const ProjectDetail = () => {
  ...
  const fetchProjectDetails = useCallback(async () => {
    try {
      ...
      const response = await axiosInstance.get(
        API_PATHS.ADMIN.PROJECT_BY_ID(id)
      );
  ...
  useEffect(() => {
    ...
    const filtered = project.activities.filter((activity) => {
      ...
      const progressStatus = (
        activity.projectStatus || "Not Started"
      ).toLowerCase();
  ...
  const getActivityBudgetStatus = (activity) => {
    if (!activity || activity.budget === null || activity.budget === undefined)
      return "balanced";
    const budget =
      typeof activity.budget === "string"
        ? parseFloat(activity.budget)
        : activity.budget;
    const expense = activity.expense || 0;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="break-words">
              ...
              <div className="text-sm sm:text-base">
                ...
                <p className="text-gray-600 break-words text-xs sm:text-sm">
                  {project.programPersonnel?.email || ""}
                </p>
              </div>
            </div>
          </div>
                    ...
                    {filteredActivities.length > 0 ? (
                      filteredActivities.map((activity, index) => {
                        const activityUtilization =
                          calculateActivityUtilization(activity);
                        const progressStatus =
                          activity.projectStatus || "Not Started";
                        ...
                        return (
                          <tr
                            ...
                            onClick={() =>
                              navigate(
                                `/admin/projects/${id}/activities/${activity.activityId}`
                              )
                            }
                          >
